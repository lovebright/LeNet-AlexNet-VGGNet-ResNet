...
#导入input_data用于自动下载和安装MNIST数据集，若无法自动下载，请手动下载好后放入MNIST_data文件夹
from tensorflow.examples.tutorials.mnist import input_data
mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)

#创建两个占位符，x为输入网络的图像，y_为输入网络的图像类别
x = tf.placeholder("float", shape=[None, 784]) #其后必有feed_dict方法
...
#-1表示自动推测这个维度的size
x_image = tf.reshape(x, [-1,28,28,1]) #占位符x中的784 = 28 * 28，实际图像：宽高28*28*1
...
y_conv = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2) #归一化成概率分布

#预测值和真实值之间的交叉墒
cross_entropy = -tf.reduce_mean(y_ * tf.log(tf.clip_by_value(y_conv,1e-10,1.0))) #因为N是常数，所以可以用tf.reduce_sum
